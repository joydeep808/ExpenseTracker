...
export const sendEmail = async (email: string ,  subject: string , html:string) => {
  ...
  const transporter = nodemailer.createTransport({
    ...
    secure:true,
    auth: {
        user: `${process.env.EMAIL}`, // Just enter Your email // in which You want to share your email
        pass:`${process.env.EMAIL_PASSWORD}`,  // You Have To Create Password  Stepts --> go to Email Click your gmail and after that manage my account and after that click on security and find App PassWord And Create A Password And Paste Here Or .env File 
    
    },
  });

  const mail = {
    from: `${process.env.EMAIL_NAME}   <${process.env.EMAIL}>`, // We can name this anything. The mail will go to your Mailtrap inbox
...
<button> <p style="background-color:#656ee8;border-radius:5px;color:#fff;font-size:16px;font-weight:bold;text-decoration:none;text-align:center;display:inline-block;width:100%;padding:10px 10px 10px 10px;line-height:100%;max-width:100%"><span><!--[if mso]><i style="letter-spacing: 10px;mso-font-width:-100%;mso-text-raise:15" hidden>&nbsp;</i><![endif]--></span><span style="max-width:100%;display:inline-block;line-height:120%;mso-padding-alt:0px;mso-text-raise:7.5px">${resetLink}</span><span><!--[if mso]><i style="letter-spacing: 10px;mso-font-width:-100%" hidden>&nbsp;</i><![endif]--></span></p>
</button>
